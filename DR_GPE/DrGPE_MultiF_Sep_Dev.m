% DrGPE_MultiF_Sep_Dev
% Dimension reduction Gausian process multi-field emulation_seperately_Developer version
% Offer an Analysis through data with given data and Method/parameters
% 
% 
% Modifications:
% WeiX, 18-1-2016, Create.
...
switch DrMethod
    ...
    case 'Isomaps'
        [Z_U,model_U] = Isomaps(U,options);
        [Z_V,model_V] = Isomaps(V,options);    
        [Z_P,model_P] = Isomaps(P,options);   
        
    case 'PCA'
%         [Z,model] = Lpca(Y,dim_new,options);
        [Z_U,model_U] = Lpca(U,dim_new,options);
        [Z_V,model_V] = Lpca(V,dim_new,options);
        [Z_P,model_P] = Lpca(P,dim_new,options);
    otherwise 
        error('No such DR method')
end
...
%     [num_X,dim_X]=size(X);
    ...
    hyp = minimize(hyp, @gp, -100, @infExact, meanfunc, covfunc, likfunc, X, Z_U(:,i));
    exp(hyp.lik);
    nlml2 = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_U(:,i));        
    [m_U(:,i) s_U(:,i)] = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_U(:,i), X_star);
end

%V
[num_Z,dim_Z]=size(Z_V);     
for i=1:dim_Z
...
%     [num_X,dim_X]=size(X);
    ...
    hyp = minimize(hyp, @gp, -100, @infExact, meanfunc, covfunc, likfunc, X, Z_V(:,i));
    exp(hyp.lik);
    nlml2 = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_V(:,i));        
    [m_V(:,i) s_V(:,i)] = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_V(:,i), X_star);
end

%P
[num_Z,dim_Z]=size(Z_P);     
for i=1:dim_Z
...
%     [num_X,dim_X]=size(X);
    ...
    hyp = minimize(hyp, @gp, -100, @infExact, meanfunc, covfunc, likfunc, X, Z_P(:,i));
    exp(hyp.lik);
    nlml2 = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_P(:,i));        
    [m_P(:,i) s_P(:,i)] = gp(hyp, @infExact, meanfunc, covfunc, likfunc, X, Z_P(:,i), X_star);
end
...
%                     Y_star = DiffusionMaps_PreImage(m(:,1:k),model,preoptions);                    
                    U_star = DiffusionMaps_PreImage(m_U(:,1:k),model_U,preoptions);
                    V_star = DiffusionMaps_PreImage(m_V(:,1:k),model_V,preoptions);
                    P_star = DiffusionMaps_PreImage(m_P(:,1:k),model_P,preoptions);               
                case 1
...
%             Y_star= Isomaps_PreImage(m(:,1:k),model,preoptions);     
            U_star = Isomaps_PreImage(m_U(:,1:k),model_U,preoptions);
            V_star = Isomaps_PreImage(m_V(:,1:k),model_V,preoptions);
            P_star = Isomaps_PreImage(m_P(:,1:k),model_P,preoptions);                         
        case 'PCA'
%             Y_star=Lpca_PreImage(m(:,1:k),model);
            U_star = Lpca_PreImage(m_U(:,1:k),model_U);
            V_star = Lpca_PreImage(m_V(:,1:k),model_V);
            P_star = Lpca_PreImage(m_P(:,1:k),model_P);             
        otherwise 
            error('No such DR method')
    end
...
figure
boxplot(MRSSE)
title(sprintf('MRSSE -%s -%s -%d Training points -SeperatedF', DrMethod, preoptions.type, num_train))
set(gca,'yscale','log');
