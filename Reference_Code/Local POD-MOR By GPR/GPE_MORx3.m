...
%% GPE_MORx3
%  GPE+MOR 3 method for 1 test case
% Modifications:
...
%%---------------Setting parameter---------------------------------------
Num_Trian=190;   % 80 is best for 'ExpData3,4.mat'
Index_test=193;  % ExpData8 use 428 426 as very good 326 fail

Num_Snapshot=10;
n_Ubases=10;     %Number of POD bases
dim_new=5;      % For LPCA more than 10 require. The new dimension of reduced emulatior
     

%% ----------------Load dataset-------------------------------------------
load('ExpData.mat')
% Num_Trian=100;
% Index_test=151;%151 is difficulty
...
%% ---------------HDM----------------------------------------------------
[Y_Full,Time_HDM]=Us_HeatDC_2D_FVM(Paras);
Time_HDM;



%% ---------------MOR Bases by perfect snapshot -----------------------------------
Y_orig_snaps=reshape(Y_orig',Paras.Num_node_x*Paras.Num_node_y,[]);

[U_OrigSnapS,S,~]=svd(Y_orig_snaps);  % U*S*V'=Rec_X
% [U_orig,S,~]=svd(T_orig);
U_OrigSnapS=U_OrigSnapS(:,1:n_Ubases);
eigenvalues=diag(S);

[Y_U_OrigSnapS,Time_U_orig]=Us_HeatDC_2D_FVM_Mor(Paras,U_OrigSnapS);
Time_U_orig;
...
%% ---------------MOR Golboal Bases -----------------------------------
Y_Global_snaps=reshape(Y',Paras.Num_node_x*Paras.Num_node_y,[]);

[U_GlobalSnapS,S,~]=svd(Y_Global_snaps);  % U*S*V'=Rec_X
% [U_orig,S,~]=svd(T_orig);
U_GlobalSnapS=U_GlobalSnapS(:,1:n_Ubases);
eigenvalues_GlobalSnapS=diag(S);

[Y_U_GlobalSnapS,Time_U_orig]=Us_HeatDC_2D_FVM_Mor(Paras,U_GlobalSnapS);
...
% --Rearrange--
Y_GPESnapS=reshape(Y_GPE',Paras.Num_node_x*Paras.Num_node_y,[]);

[U_GPESnapS,S,~]=svd(Y_GPESnapS);  % U*S*V'=Rec_X
U_GPESnapS=U_GPESnapS(:,1:n_Ubases);
eigenvalues_Ustar=diag(S);

[Y_U_GPESnapS,Time_ROM_U_GPESnapS]=Us_HeatDC_2D_FVM_Mor(Paras,U_GPESnapS);
Time_ROM_U_GPESnapS;
...
for i=1:Num_Trian
    ...
    Yi=Y(i,:);
    Yi_SnapS=reshape(Yi',Paras.Num_node_x*Paras.Num_node_y,[]);
...
% --Rearrange--
U_GPE=reshape(U_GPE',Paras.Num_node_x*Paras.Num_node_y,[]);
[Y_U_GPE,Time_ROM_U_GPE]=Us_HeatDC_2D_FVM_Mor(Paras,U_GPE);
...
L1=plot(cumsum(sum(SE_U_OrigSnapS)),'b--');
hold on
L2=plot(cumsum(sum(SE_U_GPESnapS)),'r-.');
L3=plot(cumsum(sum(SE_U_GlobalSnapS)),'g--');

legend([L1,L2,L3],'Perfect MOR L^2 Error','GPE Snapshot bases MOR L^2 Error','Global bases MOR L^2 Error')
...
L2=plot(cumsum(sum(SE_U_GPESnapS)),'r-.');
L3=plot(cumsum(sum(SE_U_GlobalSnapS)),'g--');
L4=plot(cumsum(sum(SE_U_GPE)),'y--');
% L5=plot(cumsum(sum(SE_Y_GPESnapS(:,1:Paras.t_n+1))),'m--');
...
legend([L1,L2,L3,L4],'Perfect MOR L^2 Error','GPE Snapshot bases MOR L^2 Error','Global bases MOR L^2 Error','GPE bases MOR L^2 Error')
title(sprintf('L^2 error accumulation'))
hold off
...
% ----Animation plot------------------------------------------------------
Paras.t_n=Paras.t_end/Paras.dt+1;
