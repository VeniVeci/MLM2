% function Burger1D_FEM_DBC_MOR_SolverF_Demo
%
...
% POD 
Y1_inter=Y1(2:end-1,:);
[UF,~,~]=svd(Y1_inter.^2);  % F(y)=y.^2 in Burgers' equation
UF=UF(:,1:DEIMapproximate_degree);
[phi,Uu,P] = DEIM(UF);


[Y3,T3,Time_Ode_MORDEIMsolver]=Burger1D_FEM_DBC_MOR_DEIM_SolverF(Paras,U,UF,P);
...
figure(3)
meshc(Y3)
title(sprintf('MOR-DEIM FEM Solution. Time cost= %0.3f',Time_Ode_MORDEIMsolver))
...
for i = 1:Paras.t_n+1
    ...
    plot(x,Y3(:,i))
    axis([0,1,0,1]);
    title(sprintf('MOR-DEIM FEM Animation t= %0.3f',((i-1)*(Paras.t_end/Paras.t_n))))   
