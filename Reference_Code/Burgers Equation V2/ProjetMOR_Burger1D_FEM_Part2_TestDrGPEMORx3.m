...
%%---------------Setting parameter---------------------------------------
Num_Trian=100;   % 80 is best for 'ExpData3,4.mat'
Index_test=406; % 400,403 is tracky. mostly fine. in ExpData4 404 best
...
%% ----------------Load dataset-------------------------------------------
load('ExpData4.mat') %7 is ok for HH, HH fails in 4.
...
L2=plot(x,cumsum(sum(SE_U_OrigSnapS)),'b--');
hold on
L3=plot(x,cumsum(sum(SE_U_GPESnapS)),'r-.');
L4=plot(x,cumsum(sum(SE_U_GlobalSnapS)),'g--');
L5=plot(x,cumsum(sum(SE_U_GPE)),'y--');

legend([L2,L3,L4,L5],'Perfect MOR L^2 Error','GPE Snapshot bases MOR L^2 Error','Global bases MOR L^2 Error','GPE bases MOR L^2 Error')
