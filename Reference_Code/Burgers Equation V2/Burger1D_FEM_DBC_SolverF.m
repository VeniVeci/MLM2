...
%         M(i,j)=integral(@(x)PhiPhi(x,i,j,n),0,1);
        M(i,j)=integral(@(x)(FEM_HatFunc(j,n,x).*FEM_HatFunc(i,n,x)),0,1);       
    end
end
...
for i=1:n-1
    for j=1:n-1
        B(i,j)=integral(@(x)(FEM_HatFunc(i,n,x ).*FEM_HatFunc_Diff(j,n,x )),0,1);
    end
end
...
for i=1:n-1
    for j=1:n-1
        C(i,j)=integral(@(x)(FEM_HatFunc_Diff(i,n,x ).*FEM_HatFunc_Diff(j,n,x)),0,1);
    end
end
...
%         G(i,1)=integral(@(x)PhiU0(x,i,n,u0a,u0b),0,1);
        G(i,1)=integral(@(x)(U0(x,u0a,u0b).*FEM_HatFunc(i,n,x)),0,1);       
end
