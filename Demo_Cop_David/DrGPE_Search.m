...
d_X=[6:2:20];         %Number of coefficients used for input
...
index=1;
figure
field=reshape(Y_star(index,:),[64,64]);
surf(field)
contourf(field,8)
colorbar
...
figure
field=reshape(Y_starorig(index,:),[64,64]);
surf(field)
contourf(field,8)
colorbar
