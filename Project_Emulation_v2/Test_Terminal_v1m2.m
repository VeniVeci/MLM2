...
%         options = struct('dim_new',new_dim,'neighbor',10,'d2p_method','Dw', 'd2p_Dwpara',10,'d2p_points',10);      
     ...
     if DiffusionMap.active==1;
        DiffusionMap.options.dim_new=new_dim;     
%         options = struct('dim_new',new_dim,'neighbor',10,'d2p_method','Dw', 'd2p_Dwpara',10,'d2p_points',10);    
        [ DiffusionMap.options.kpara ] = DiffusionMap_AutoK(Y,DiffusionMap.options);
...
if boxplot_RecRateErr.active==1
 ...
 %% Line plot
 if lineplot.active==1
   index = randi([1 num_test],3,1);
%      index =[49,45,65]
   ...
   if lpca.active==1;
       ...
       hold on 
       plot(Y_star_svd(index,:)','k-')
       hold off   
       title(sprintf('LPCA-GPE Actual line plot compare.'));
       plot_cursor=plot_cursor+1;
   end
   ...
   if kpca.active==1;
       ...
       plot(Y_starorig(index,:)','k:')
       hold on 
       plot(Y_star_kpca(index,:)','k-')
       hold off   
       title(sprintf('KPCA-GPE Actual line plot compare.'));
       plot_cursor=plot_cursor+1;
   end
   ...
   if isomap.active==1;
       ...
       plot(Y_starorig(index,:)','k:')
       hold on 
       plot(Y_star_isomap(index,:)','k-')
       hold off   
       title(sprintf('Isomaps-GPE Actual line plot compare.'));
       plot_cursor=plot_cursor+1;
   end
   ...
   if DiffusionMap.active==1;
       ...
       plot(Y_starorig(index,:)','k:')
       hold on 
       plot(Y_star_DiffusionMap(index,:)','k-')
       hold off   
       title(sprintf('DiffusionMaps-GPE Actual line plot compare.'));
       plot_cursor=plot_cursor+1;
   end
 end
