...
num_train_ref=[40:40:120]'; % must be in INCREASING order
num_test=300;
...
%     model_Rec(i)=model;
    ...
    for k=1:dim_new
        ...
        switch DrMethod    
            case 'kPCA'      
                Z_star= ANN_prediction_crossvalidation(X',Z(:,1:k)',X_star',kfold);
                Y_star = Kpca_PreImage(Z_star',model,preoptions);
                                
            case 'DiffusionMaps'
                Z_star= ANN_prediction_crossvalidation(X',Z(:,1:k+1)',X_star',kfold);
