...
figure
boxplot(RE_P)
title(sprintf('SVM P Error rate -%s -%d Training points', DrMethod, num_train))
set(gca,'yscale','log');
...
ylabel('Error rate')
legend('U','V','P');
title(sprintf('SVM Rate Error -%s -%d Training points', DrMethod, num_train))
...
figure
DisplayUVP_v2( field_U,field_V,field_P,Re,tf,dt,lx,ly,nx,ny,BC )
title(sprintf('%s SVM Predicted field -%d Training points Index=%d ', DrMethod,num_train,Test_split+index-1));
