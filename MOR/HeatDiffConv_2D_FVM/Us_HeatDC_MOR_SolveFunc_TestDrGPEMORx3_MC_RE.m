%% Us_HeatDC_MOR_SolveFunc_TestDrGPEMORx3_MC_RE
%
% Modifications:
% 5-4-2016, WeiX, first edition, Us_HeatDC_MOR_SolveFunc_TestDrGPEMORx3_MC
% 9-9-2016, WeiX, add error sort, Us_HeatDC_MOR_SolveFunc_TestDrGPEMORx3_MC
% 9-9-2016, WeiX, add relative error, Us_HeatDC_MOR_SolveFunc_TestDrGPEMORx3_MC_RE
...
%     uParas.ampy=1;
    ...
    Y_orig_snaps =reshape(Y_starorig(i,:)',Paras.Num_node_y*Paras.Num_node_x,[]);
    ...
    [Y_U_OrigSnapS,~]=Us_HeatDC_MOR_SolveFunc(Paras,uParas,X_int,U_OrigSnapS);
...
%     uParas.ampy=1;
    ...
    [Y_U_GlobalSnapS,~]=Us_HeatDC_MOR_SolveFunc(Paras,uParas,X_int,U_GlobalSnapS);
...
%     eigenvalues_Ustar=diag(S);
    [Y_U_GPESnapS,~]=Us_HeatDC_MOR_SolveFunc(Paras,uParas,X_int,U_GPESnapS);
...
%     uParas.ampy=1;
    ...
    U_GPEi=reshape(U_GPE(i,:)',Paras.Num_node_y*Paras.Num_node_x,[]);

    [Y_U_GPE,~]=Us_HeatDC_MOR_SolveFunc(Paras,uParas,X_int,U_GPEi);
...
for i =1:Num_Test
    ...
    RE_U_OrigSnapS(i,:)= mean(sum((Y_U_OrigSnapS_Rec(:,:,i)-Y_Rec(:,:,Test_StartIndex+i-1)).^2,1) ./ sum(Y_Rec(:,:,Test_StartIndex+i-1).^2,1) ) ;
    RE_U_GlobalSnapS(i,:)= mean(sum((Y_U_GlobalSnapS_Rec(:,:,i)-Y_Rec(:,:,Test_StartIndex+i-1)).^2,1) ./ sum(Y_Rec(:,:,Test_StartIndex+i-1).^2,1) ) ;
    RE_U_GPESnapS(i,:)= mean(sum((Y_U_GPESnapS_Rec(:,:,i)-Y_Rec(:,:,Test_StartIndex+i-1)).^2,1) ./ sum(Y_Rec(:,:,Test_StartIndex+i-1).^2,1) ) ;
    RE_U_GPE(i,:)= mean(sum((Y_U_GPE_Rec(:,:,i)-Y_Rec(:,:,Test_StartIndex+i-1)).^2,1) ./ sum(Y_Rec(:,:,Test_StartIndex+i-1).^2,1) ) ;
...
% [SSE_dxdt_U_GPESnapS_Sort,SSE_dxdt_U_GPESnapS_Sortindex]=sort(SSE_dxdt_U_GPESnapS);
% meanval=mean(SSE_dxdt_U_GPESnapS)
