%% ProjetMOR_Burger1D_FEM_Part2_TestDrGPEMORx3_PODn_SCV2
%  version 2: using different dataset formate. Single case U
% Modifications:
% 13-Sep-2016, WeiX, first edition 
...
%% ----------------Load dataset-------------------------------------------
% load('ExpDataV2_22.mat') %7 is ok for HH, HH fails in 4.
...
for i =1:Num_Test
        ...
        U_GPESnapSj=U_GPESnapS(:,1:n_Ubases);
        [Y_U_GPESnapS,~,~]=Burger1D_FEM_DBC_MOR_SolverF(Paras,U_GPESnapSj);
        ...
        SSE_dxdt_U_GPESnapS(i,:)=sum(SSE_dx_U_GPESnapS(i,:),2);           
...
% title(sprintf('L^2 Error Boxplot. %s, Num_{Trian}=%0i, Num_{Test}=%0i, Num_{Snapshot}=%0i, Num_{DimNew}=%i', DrMethod,Num_Train,Num_Test,Num_Snapshot,dim_new))
% set(gca,'yscale','log');
...
[SortSSE_dxdt_U_GPESnapS,SortIndex]=sort(SSE_dxdt_U_GPESnapS);
...
figure 
hold on 
for i = 1:Paras.t_n/number_show:Paras.t_n+1 
    plot(x,y_orig(:,i),'k:','LineWidth',1)
    plot(x,y_GPE_DEIM(:,i),'k-','LineWidth',1)

end 
box on
hold off

title(sprintf('Actual plot case %0i for %s Num_{Trian}=%0i, Num_{Test}=%0i, Num_{Snapshot}=%0i, Num_{DimNew}=%i POD=%i', Test_index, DrMethod,Num_Train,Num_Test,Num_Snapshot,dim_new,n_Ubases))
...
ylabel({'u'},'FontSize',30,'FontName','Times New Roman','FontWeight','normal');
...
title('(d)','FontSize',30,'FontName','Times New Roman','FontWeight','normal')
% 

% ax = gca; 
% ax.YLabel.Position=[-0.05,0.05,-1];
% ax.YLabel.Position=[-0.07,0.07,-1];
% ax.YTick=[0:0.05:0.2];
% ax.XTick=[0:0.5:1];
