%% ProjetMOR_Burger1D_FEM_Part2_TestDrGPEMORx3_PODn_DEIM_SCV2
...
for i =1:Num_Test
        ...
        for j=n_Ubases:n_Ubases
        
            U_GPESnapSj=U_GPESnapS(:,1:j);
            U_DEIMj=U_DEIM(:,1:j);
            Pj=P(:,1:j);            %same number as n_Ubases
                 
            [Y_U_GPESnapS,~,~]=Burger1D_FEM_DBC_MOR_DEIM_SolverF(Paras,U_GPESnapSj,U_DEIMj,Pj);
        
            Y_U_GPESnapS_Rec(:,:,i,j)=Y_U_GPESnapS;
        
            SSE_dx_U_GPESnapS(i,:)=sum((Y_U_GPESnapS_Rec(:,:,i,j)-Y_Rec(:,:,Test_StartIndex+i-1)).^2,1);   %Square sum error; integral on dx
...
title(sprintf('L^2 Error Boxplot. %s+DEIM, Num_{Trian}=%0i, Num_{Test}=%0i, Num_{Snapshot}=%0i,DEIM_{Snapshot}=%0i, Num_{DimNew}=%i', DrMethod,Num_Train,Num_Test,Num_Snapshot,dim_new))
set(gca,'yscale','log');
...
figure
boxplot(RE_U_GPESnapS(:,n_Ubases))
