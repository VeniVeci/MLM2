%% ProjetMOR_Burger1D_FEM_Part2_TestDrGPEMORx3V2
%  V2 for new data formate   
%
% Modifications:
% 29-3-2016, WeiX, first edition 

clear

%%---------------Setting parameter---------------------------------------
Num_Train=180;   % 80 is best for 'ExpData3,4.mat'
Index_test=326;  % ExpData8 use 428 426 as very good 326 fail 385
...
%% ----------------Load dataset-------------------------------------------
load('ExpDataV2_19.mat') %7 is ok for HH, HH fails in 4.
...
Y_orig_snaps=Y_Rec(:,Index_snapshot,Index_test);
% n_Ubases=5;
...
[U_GPE,Time_Emu]=Func_DrGPE(X(1:Num_Train,:),U_Y(1:Num_Train,:),X_star,options,preoptions);     
