function [Y,T,Time_Ode_solver]=burgerSolver(Paras,g,u0)
%%
...
%     F(i,1)=integral(@(x)(0.01*exp(x).*FEM_HatFunc(i,n,x)),0,1);    
    F(i,1)=integral(@(x)(g(x).*FEM_HatFunc(i,n,x)),0,1);
end
...
tic;
[T,Y] = ode45(@(t,y) ode(t,y,M,B,C,F,v),t,y_int); % Solve ODE
Time_Ode_solver=toc;
