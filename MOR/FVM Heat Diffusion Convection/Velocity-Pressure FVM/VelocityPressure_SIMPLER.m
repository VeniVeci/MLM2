%% uvp SIMPLER solver -------warming Fail-------------------

% ---------------

%Simulating the Velosity and Pressure field 2D equation  
...by the Finite Volume Method using SIMPLER algorithm 
...
% -----------------------Solver Parameters---------------------------
grid_Num_x=31;        % number of volumes on x direction
grid_Num_y=31;        % number of volumes on y direction
...
for i=1:MaxIteration
    ...
    Grid_pseudo=Grid;
    [u_hat,Grid_pseudo] = RetanG_pseudou(Grid_pseudo);
    [v_hat,Grid_pseudo] = RetanG_pseudov(Grid_pseudo);
    Grid_pseudo.u=u_hat;
    Grid_pseudo.v=v_hat;
    p_star = RetanG_solverp(Grid_pseudo,'penta'); 
    Grid.p=p_star;
...
streamline(corx',cory',uplot',vplot',rand(grid_Num_x,1),rand(grid_Num_x,1));
hold off
