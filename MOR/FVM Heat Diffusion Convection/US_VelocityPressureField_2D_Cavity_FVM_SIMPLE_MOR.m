...
[Uu,S,~]=svd(Rec_u);  % U*S*V'=Rec_X
Uu=Uu(:,1:approximate_degree);
eigenvalues_u=diag(S);

[Uv,S,~]=svd(Rec_v);  % U*S*V'=Rec_X
Uv=Uv(:,1:approximate_degree);
eigenvalues_v=diag(S);
...
for t=0:dt:t_end
    ...
    A_Mor=Uu'*A*Uu;
    B_Mor=Uu'*B;
    u_temp_Mor=A_Mor\B_Mor;
    u_temp=Uu*u_temp_Mor;
...
%     Rec_u=[Rec_u,u_temp];
    ...
    A_Mor=Uv'*A*Uv;
    B_Mor=Uv'*B;
    v_temp_Mor=A_Mor\B_Mor;
    v_temp=Uv*v_temp_Mor;
