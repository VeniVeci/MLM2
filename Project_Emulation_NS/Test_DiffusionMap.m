...
%% Dataset Parameter
Name_field='exp1';
num_train=10;           %30;80 for exp1      %40 is bad  %Fine:50  %Best:80 
num_test=300;
...
%     [Y_star_DiffusionMap,~,t_DiffusionMap]=GPR_DiffusionMap(X,Y,X_star,DiffusionMap.options,DiffusionMap.Reoptions);
    ...
    DiffusionMap.options.kpara  = DiffusionMap_AutoK(U,DiffusionMap.options)*1;
    [U_star_DiffusionMap,~,~]=GPR_DiffusionMap(X,U,X_star,DiffusionMap.options,DiffusionMap.Reoptions);
    
    DiffusionMap.options.kpara = DiffusionMap_AutoK(V,DiffusionMap.options)*1;
    [V_star_DiffusionMap,~,~]=GPR_DiffusionMap(X,V,X_star,DiffusionMap.options,DiffusionMap.Reoptions);
    
    DiffusionMap.options.kpara = DiffusionMap_AutoK(P,DiffusionMap.options)*1;
...
%    index=rand(5,1);
   ...
   figure(plot_cursor)
   plot(U_star_DiffusionMap(index,:)',U_starorig(index,:)')
   plot_cursor=plot_cursor+1;      
   ...
   hold off
   title(sprintf('Coordinate to line; U field; Solid line for real case, dash for prediction'))
   plot_cursor=plot_cursor+1;      
   ...
   hold off
   title(sprintf('Coordinate to line; V field; Solid line for real case, dash for prediction'))
   plot_cursor=plot_cursor+1;      
   ...
   hold off
   title(sprintf('Coordinate to line; P field; Solid line for real case, dash for prediction'))
   plot_cursor=plot_cursor+1;      
