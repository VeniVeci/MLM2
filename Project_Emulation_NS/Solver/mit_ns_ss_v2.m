function [U,V,P]=  mit_ns_ss(Re,dt,tf,lx,ly,nx,ny,nsteps,SSlimit,BC)
% Solves the incompressible Navier-Stokes equations steady state.
% Modified from code by  Benjamin Seibold http://www-math.mit.edu/~seibold/
%
% One can assign many moving B.C. with v2
%
%------------------------
...
% boundary conditions
%     uN = x*0+BC.uN;    vN = avg(x)*0;
%     uS = x*0;      vS = avg(x)*0;
%     uW = avg(y)*0; vW = y*0;
%     uE = avg(y)*0; vE = y*0;

uN = x*0+BC.uN;      vN = avg(x)*0+BC.vN;
uS = x*0+BC.uS;      vS = avg(x)*0+BC.vS;
uW = avg(y)*0+BC.uW; vW = y*0+BC.vW;
uE = avg(y)*0+BC.uE; vE = y*0+BC.vE;
%-----------------------------------------------------------------------
...
%    Umax_old=max(U_old(:));
%    Vmax_old=max(V_old(:));
%    Uerrate(k)= abs((Umax-Umax_old)/1);
   
%    Usum=sum(U(:));
%    Usum_old=sum(U_old(:));
%    Uerrate(k)= abs((Usum-Usum_old)/Usum);


   % Record err between iterations
   U_err=abs((U-U_old)./U);
   Uerrate(k)=sum(U_err(:))/length(U_err(:));
   
   V_err=abs((V-V_old)./V);
   Verrate(k)=sum(V_err(:))/length(V_err(:));  
   
   U_old=U;
   V_old=V;
   
   if ((Uerrate(k)<SSlimit) & (Uerrate(k)<SSlimit))
       break 
   end
